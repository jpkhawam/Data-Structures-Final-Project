...
std::vector<int> customSort(const std::vector<int> &input) {
    ...
    //each element in this array represents the number of occurences of
    //the number 0<i<27 from input
    std::vector<int> helper_array(max_value + 1, 0);

    //complexity O(n)
    for (int index = 0; index < size; index++) {
        //helper_array[i] stores the number of occurences the number i appears
        //passes once on each element on input, so O(n)
        helper_array.at(input.at(index))++;
    }

    //the outer for loop runs only 26 times
    for (int index = 1; index < max_value + 1; index++) {
